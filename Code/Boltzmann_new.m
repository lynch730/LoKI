...
%       end
      ...
      eedfTimeDerivative(0, [auxEedf auxNe]', boltzmann, true, eDensIsTimeDependentAux, 0);    % flush persistent variables previously stored in function memory
      ...
      if GUIisActive
        ...
        % Last arg is extra for indicating jacobian
        % first 0 is no flush, second is time-dependent flag, third is
        % jacobiab flag
        Fac = @(t, x) eedfTimeDerivative(t, x, boltzmann, 0, 1, 0);
        Jac = @(t, x) eedfTimeDerivative(t, x, boltzmann, 0, 1, 1);
        odeOptionsLocal.Jacobian = Jac;
        odeOptionsLocal.rtol = 1e-6;
        odeOptionsLocal.atol = 1e-20;
    ...
    [finalTimes,solutions] = ode15s(Fac, times, [auxEedf auxNe]', odeOptionsLocal);
      finalEedfs = solutions(:,1:end-1);
      finalNe =  solutions(:,end);
% 
...
% %         [timesSol,eedfsSol,~,~,~] = ...
%         [timesSol,solutions] = ode15s(Fac, ...
%                    times(initialTimeIdx:finalTimeIdx), [auxEedf auxNe]', ...
%                     odeOptionsLocal);
%         % separate solutions
%         eedfsSol = solutions(:,1:end-1);
%         neSol = solutions(:,end);
%         % renormalize eedfs of the current chunck
%         norm_block = zeros(length(timesSol), 1);
%         for idx = 1:length(timesSol)
%           norm_block(idx) = (boltzmann.energyGrid.step*dot(eedfsSol(idx,:), sqrt(boltzmann.energyGrid.cell)));
%           eedfsSol(idx,:) = eedfsSol(idx,:)/norm_block(idx);
%         end
...
%       
      ...
      mean_energy = zeros(length(finalTimes), 1);
%       pp=plot(boltzmann.energyGrid.cell,  boltzmann.energyGrid.cell*NaN);
        set(gca, 'XScale', 'log')
%         set(gca, 'YScale', 'log')
%         ylim([1e-40 1e10])
%         xlim([1e-2 100])
      ...
      for idx = 1:length(finalTimes)
        ...
        eedfTimeDerivative(finalTimes(idx), [finalEedfs(idx,:) finalNe(idx)]', boltzmann, false, eDensIsTimeDependentAux, 0);
        ...
        mean_energy(idx) = boltzmann.swarmParam.meanEnergy;
    
%         pp.XData = boltzmann.energyGrid.cell;
%         pp.YData = boltzmann.eedf;
% 
%         drawnow;
        
      end

      plot(finalTimes, mean_energy)
    ...
    function eedf = temporalIntegration(boltzmann)
      ...
      eedfTimeDerivative(0, [eedf boltzmann.workCond.electronDensity]', boltzmann, true, false, 1); % flush persistent variables previously stored in function memory
...
function derivatives = eedfTimeDerivative(time,variables,boltzmann,clearPersistentVars, electronDensityIsTimeDependent, Jac_flag)
...
% time is given by eedf. 
  ...
  if boltzmann.isTimeDependent
    EoN = boltzmann.pulseFunction(time,boltzmann.pulseFunctionParameters);
%     fprintf('\n %f', EoN)
    EoN = EoN*1e-21;
  else
  ...
  if isempty(matrix)
    ...
    if ~exist('Jac_matrix', 'var') || isempty(Jac_matrix)
        Jac_matrix = N*sqrt(2*e/me)*matrix./sqrt(energyCell');
        Jac_matrix = [Jac_matrix; ones(1, Ne)];
        Jac_matrix = [Jac_matrix, zeros(Ne+1, 1)];
        Jac_matrix = sparse(Jac_matrix);
    end
    ...
    if boltzmann.includeNonConservativeIonization || boltzmann.includeNonConservativeAttachment
      ...
      switch boltzmann.eDensGrowthModel
        case 'temporal'
          ...
          growthMatrixDiagElements = -gam*sqrt(energyCell);
        case 'spatial'
           error('not setup for spatial')
      end
    end
...
%     CIEff = sqrt(2*e/me)*energyStep*dot(eedf,CIEffIntegrand);
    CIEFF_jac = gam*energyStep*(CIEffIntegrand');
    CIEff = CIEFF_jac * eedf;
    ...
    switch boltzmann.eDensGrowthModel
      case 'temporal'
        ...
        WoN = 1e-16;
        gtmp = energyNode./( 3*energyStep^2*totalCrossSectionMod.* ...
        ...
        boltzmann.g_fieldTemporalGrowth = gtmp*energyStep^2;
        % evaluate time derivative of each discrete component of the eedf (case with temporal growth models)
%         dfdt = N*gam*( ( ... % multiplicative constant to obtain proper units of time
%           matrix*eedf + ...           % full basic boltzmann matrix
%           EoN2*( -gtmp(id)-gtmp(2:cellNumber+1) + ... % time dependent diagonal component
%           CIEff*growthMatrixDiagElements)'.*eedf + ...
%           EoN2*[ 0 gtmp(2:cellNumber) ]'.*[ 0; eedf(idm) ] + ...  % time dependent inf. diagonal component
%           EoN2*[ gtmp(2:cellNumber) 0 ]'.*[ eedf(2:cellNumber); 0 ] ...      % time dependent sup. diagonal component
%           )./sqrt(energyCell'));    % divide by the square root of the energy to obtain derivative of the EEDF
        ...
        if Jac_flag 
            ...
            % Term one, Jacobian left 
            Md(:,1) = EoN2*[gtmp(2:cellNumber), 0]';
            Md(:,1) = Md(:,1)./sqrt([energyCell(2:cellNumber), 1.0]');
            
            % Term one, Jacobian diagonal
            Md(:,2) = EoN2*( -gtmp(id)-gtmp(2:cellNumber+1) + CIEff*growthMatrixDiagElements)';
            Md(:,2) = Md(:,2)./sqrt(energyCell');
            
            % Term one, Jacobian right 
            Md(:,3) = EoN2*[0, gtmp(2:cellNumber)]';
            Md(:,3) = Md(:,3)./sqrt([1.0, energyCell(2:cellNumber)]');
    
 
            Md = [Md; [0, 0, 0]];

            derivatives = spdiags(spdiags(Jac_matrix, -1:1) + N.*gam.*Md, -1:1, Jac_matrix);
            derivatives(end, 1:end-1) = boltzmann.workCond.gasDensity*CIEFF_jac;

        else

            derivatives = Jac_matrix * [eedf; ne];
            ...
            derivatives(1:Ne, 1) = derivatives(1:Ne, 1) + dfdt;
            derivatives(end) = ne*boltzmann.workCond.gasDensity*CIEff;

        end
        
        %         M_diag() = M_diag + EoN2*[ 0 gtmp(2:cellNumber) ]'.*[ 0; eedf(idm) ] ;
%         M_diag = M_diag + EoN2*[ gtmp(2:cellNumber) 0 ]'.*[ eedf(2:cellNumber); 0 ];

        

      case 'spatial'
        error('Not setup for spatial case')
    end
  else
    % evaluate time derivative of each discrete component of the eedf (case without growth models)
    error('Not setup for nonconservative')
  end
  
  % evaluate e-e contribution to the time derivative (time dependent) of each discrete component of the eedf
  if boltzmann.includeEECollisions
    error('not setup for ee')
  end
